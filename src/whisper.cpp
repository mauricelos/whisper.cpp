...
static const std::map<e_model, size_t> MEM_REQ_MODEL = {
    {MODEL_TINY, 74ull * MB},     {MODEL_BASE, 142ull * MB},   {MODEL_SMALL, 466ull * MB},
    {MODEL_MEDIUM, 1464ull * MB}, {MODEL_LARGE, 2952ull * MB},
};

static const std::map<e_model, size_t> MEM_REQ_MEMORY = {
    {MODEL_TINY, 12ull * MB},    {MODEL_BASE, 24ull * MB},   {MODEL_SMALL, 70ull * MB},
    {MODEL_MEDIUM, 184ull * MB}, {MODEL_LARGE, 306ull * MB},
};

static const std::map<e_model, size_t> MEM_REQ_ENCODE = {
    {MODEL_TINY, 80ull * MB},    {MODEL_BASE, 128ull * MB},   {MODEL_SMALL, 300ull * MB},
    {MODEL_MEDIUM, 680ull * MB}, {MODEL_LARGE, 1100ull * MB},
};

static const std::map<e_model, size_t> MEM_REQ_ENCODE_LAYER = {
    {MODEL_TINY, 104ull * MB},   {MODEL_BASE, 138ull * MB},  {MODEL_SMALL, 208ull * MB},
    {MODEL_MEDIUM, 280ull * MB}, {MODEL_LARGE, 354ull * MB},
};

static const std::map<e_model, size_t> MEM_REQ_DECODE = {
    {MODEL_TINY, 200ull * MB},   {MODEL_BASE, 202ull * MB},  {MODEL_SMALL, 204ull * MB},
    {MODEL_MEDIUM, 206ull * MB}, {MODEL_LARGE, 208ull * MB},
};

static const std::map<e_model, size_t> MEM_REQ_DECODE_LAYER = {
    {MODEL_TINY, 32ull * MB},   {MODEL_BASE, 44ull * MB},   {MODEL_SMALL, 64ull * MB},
    {MODEL_MEDIUM, 84ull * MB}, {MODEL_LARGE, 110ull * MB},
};
...
static bool whisper_model_load(const std::string& fname, whisper_context& wctx)
{
    ...
    // load hparams
    {
        ...
        wctx.buf_compute_layer.resize(
            std::max(MEM_REQ_ENCODE_LAYER.at(model.type), MEM_REQ_DECODE_LAYER.at(model.type)));
    }
...
static bool whisper_encode(whisper_context& wctx, const int n_threads, const int mel_offset)
{
    ...
    for (int il = 0; il < n_layer; ++il)
    {
        ...
        // norm
        {
            ...
            cur = ggml_add(ctxL, ggml_mul(ctxL, ggml_repeat(ctxL, layer.attn_ln_0_w, cur), cur),
        ...
        // self-attention
        {
            ...
            struct ggml_tensor* Q = ggml_permute(
                ctxL, ggml_cpy(ctxL, Qcur, ggml_new_tensor_3d(ctxL, GGML_TYPE_F16, n_state / n_head, n_head, n_ctx)), 0,
                2, 1, 3);

            struct ggml_tensor* K = ggml_permute(
                ctxL, ggml_cpy(ctxL, Kcur, ggml_new_tensor_3d(ctxL, GGML_TYPE_F16, n_state / n_head, n_head, n_ctx)), 0,
                2, 1, 3);

            struct ggml_tensor* V = ggml_cpy(
                ctxL, ggml_permute(ctxL, ggml_reshape_3d(ctxL, Vcur, n_state / n_head, n_head, n_ctx), 1, 2, 0, 3),
                ggml_new_tensor_3d(ctxL, GGML_TYPE_F16, n_ctx, n_state / n_head, n_head));

            struct ggml_tensor* KQV = ggml_flash_attn(ctxL, Q, K, V, false);
#else
            struct ggml_tensor* Q = ggml_permute(
                ctxL, ggml_cpy(ctxL, Qcur, ggml_new_tensor_3d(ctxL, GGML_TYPE_F32, n_state / n_head, n_head, n_ctx)), 0,
                2, 1, 3);
            ...
            struct ggml_tensor* V = ggml_cpy(
                ctxL, ggml_permute(ctxL, ggml_reshape_3d(ctxL, Vcur, n_state / n_head, n_head, n_ctx), 0, 2, 1, 3),
        ...
        // feed-forward network
        {
            // norm
            {
                ...
                cur = ggml_add(ctxL, ggml_mul(ctxL, ggml_repeat(ctxL, layer.mlp_ln_w, cur), cur),
                               ggml_repeat(ctxL, layer.mlp_ln_b, cur));
            }

#ifdef USE_FLASH_FF
            cur = ggml_flash_ff(ctxL, ggml_cpy(ctxL, cur, ggml_new_tensor_2d(ctxL, GGML_TYPE_F16, n_state, N)),
    ...
    // norm
    {
        ...
        cur = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.e_ln_w, cur), cur),
    ...
    // pre-compute cross-attention memory
    {
        ...
        for (int il = 0; il < model.hparams.n_text_layer; ++il)
        {
            ...
            struct ggml_tensor* v = ggml_view_1d(ctx0, model.memory_cross_v, n_state * n_ctx,
...
//
static bool whisper_decode(whisper_context& wctx, const int n_threads, const whisper_token* tokens, const int n_tokens,
                           const int n_past)
{
    const auto& model = wctx.model;
    const auto& hparams = model.hparams;
    ...
    for (int il = 0; il < n_layer; ++il)
    {
        ...
        // self-attention
        {
            ...
            // store key and value to memory
            {
                struct ggml_tensor* k =
                    ggml_view_1d(ctxL, model.memory_k, N * n_state,
                                 (ggml_element_size(model.memory_k) * n_state) * (il * n_ctx + n_past));
                struct ggml_tensor* v =
                    ggml_view_1d(ctxL, model.memory_v, N * n_state,
            ...
            struct ggml_tensor* Q = ggml_permute(
                ctxL, ggml_cpy(ctxL, Qcur, ggml_new_tensor_3d(ctxL, GGML_TYPE_F32, n_state / n_head, n_head, N)), 0, 2,
                1, 3);

            struct ggml_tensor* K =
                ggml_permute(ctxL,
                             ggml_reshape_3d(ctxL,
                                             ggml_view_1d(ctxL, model.memory_k, (n_past + N) * n_state,
            ...
            struct ggml_tensor* V_trans =
                ggml_permute(ctxL,
                             ggml_reshape_3d(ctxL,
                                             ggml_view_1d(ctxL, model.memory_v, (n_past + N) * n_state,
        ...
        // norm
        {
            ...
            // cur = ln_0_w*cur + ln_0_b
            cur = ggml_add(ctxL, ggml_mul(ctxL, ggml_repeat(ctxL, layer.cross_attn_ln_0_w, cur), cur),
        ...
        // cross-attention
        {
            ...
            struct ggml_tensor* Kcross =
                ggml_reshape_3d(ctxL,
                                ggml_view_1d(ctxL, model.memory_cross_k, M * n_state,
                                             il * M * ggml_element_size(model.memory_cross_k) * n_state),
                                n_state / n_head, n_head, M);

            struct ggml_tensor* Vcross =
                ggml_reshape_3d(ctxL,
                                ggml_view_1d(ctxL, model.memory_cross_v, M * n_state,
    ...
    // norm
    {
        ...
        cur = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.d_ln_w, cur), cur),
...
static whisper_token_data whisper_sample_best(const whisper_vocab& vocab, const float* probs, bool force_timestamp,
                                              bool is_initial)
{
    ...
    std::partial_sort(probs_id.begin(), probs_id.begin() + top_k, probs_id.end(),
...
static bool log_mel_spectrogram(const float* samples, const int n_samples, const int sample_rate, const int fft_size,
                                const int fft_step, const int n_mel, const int n_threads,
                                const whisper_filters& filters, const bool speed_up, whisper_mel& mel)
{
...
int whisper_pcm_to_mel(struct whisper_context* ctx, const float* samples, int n_samples, int n_threads)
{
    ...
    if (!log_mel_spectrogram(samples, n_samples, WHISPER_SAMPLE_RATE, WHISPER_N_FFT, WHISPER_HOP_LENGTH, WHISPER_N_MEL,
                             n_threads, ctx->model.filters, false, ctx->mel))
    {
...
int whisper_pcm_to_mel_phase_vocoder(struct whisper_context* ctx, const float* samples, int n_samples, int n_threads)
{
    ...
    if (!log_mel_spectrogram(samples, n_samples, WHISPER_SAMPLE_RATE, 2 * WHISPER_N_FFT, 2 * WHISPER_HOP_LENGTH,
                             WHISPER_N_MEL, n_threads, ctx->model.filters, true, ctx->mel))
    {
        fprintf(stderr, "%s: failed to compute mel spectrogram\n", __func__);
        return -1;
    }
...
static void whisper_exp_compute_token_level_timestamps(struct whisper_context* ctx, int i_segment, float thold_pt,
                                                       float thold_ptsum);
...
static int whisper_wrap_segment(struct whisper_context* ctx, int max_len)
{
    ...
    for (int i = 0; i < (int)segment.tokens.size(); i++)
    {
        ...
        if (acc + cur > max_len && i > 0)
        {
            ...
            ctx->result_all.back().tokens.insert(ctx->result_all.back().tokens.end(), segment.tokens.begin() + i,
                                                 segment.tokens.end());
    ...
    while (true)
    {
        ...
        for (int i = 0, n_max = whisper_n_text_ctx(ctx) / 2 - 4; i < n_max; ++i)
        {
            ...
            //
            {
                ...
                //    printf("%s: %10s %6d %6.3f '%s'\n", __func__, tt.c_str(), token.id, token.pt,
            ...
            for (int i = 0; i < (int)tokens_cur.size(); i++)
            {
                ...
                if (tokens_cur[i].id > whisper_token_beg(ctx) && !params.single_segment)
                {
                    ...
                    if (!text.empty())
                    {
                        ...
                        if (params.token_timestamps)
                        {
                            whisper_exp_compute_token_level_timestamps(ctx, result_all.size() - 1, params.thold_pt,
                                                                       params.thold_ptsum);
